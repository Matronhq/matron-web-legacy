...
function onBackToAppClick(): void {
    ...
    document.cookie = "matron_mobile_redirect_to_guide=false;path=/;max-age=14400";
...
function renderConfigError(message: string): void {
    ...
    message = `<h2>Error loading Matron</h2><p>${message}</p><p>${contactMsg}</p>`;
...
async function initPage(): Promise<void> {
    ...
    let deepLinkUrl = `https://mobile.matron.chat${metadata.deepLinkPath}`;
    ...
    // Not part of updateMobilePage as the link is only shown on mobile_guide
    document.getElementById("back_to_element_button")!.onclick = onBackToAppClick;
