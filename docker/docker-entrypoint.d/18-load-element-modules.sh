...
mkdir -p /tmp/matron-web-config
cp /app/config*.json /tmp/matron-web-config/
...
if [ -d "/modules" ] && [ "$( ls -A '/modules' )" ]; then
    ...
    do
        ...
        jq ".modules += [\"/modules/$MODULE/$ENTRYPOINT\"]" /tmp/matron-web-config/config.json | sponge /tmp/matron-web-config/config.json
    done
fi
